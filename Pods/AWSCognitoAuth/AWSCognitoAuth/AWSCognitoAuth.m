...
- (void) launchSignInVC: (UIViewController *) vc {
	...
	NSString *url = [NSString stringWithFormat:@"%@/oauth2/authorize?response_type=token&client_id=%@&state=%@&redirect_uri=%@&scope=%@&code_challenge=%@&code_challenge_method=S256%@",self.authConfiguration.webDomain, self.authConfiguration.appClientId, self.state,[self urlEncode:self.authConfiguration.signInRedirectUri], [self urlEncode:[self normalizeScopes]], self.proofKeyHash, suffix];
//	NSString *url = [NSString stringWithFormat:@"%@/oauth2/authorize?response_type=token&client_id=%@&state=%@&redirect_uri=%@",self.authConfiguration.webDomain, self.authConfiguration.appClientId,self.state,[self urlEncode:self.authConfiguration.signInRedirectUri]];
